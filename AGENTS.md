...
现有组件不是只能按当前实现使用的封闭成品，也不是必须强行复用的约束。遇到组件基本适用但缺少少量能力时，按以下顺序处理：

1. 优先为现有组件增加语义通用的可选 props。
2. 新增 props 必须向后兼容，默认值保持现有业务行为，不应把某个页面的业务字段或流程泄漏到通用组件中。
3. 为新增 props 补充类型、实现和测试，并确认不会改变其他调用方的布局、交互、性能和无障碍行为。
4. 如果需求会改变组件的核心职责、引入明显业务耦合、产生大量相互依赖的配置，或现有组件的基础假设不成立，再新增职责明确的组件；不要为了复用而继续堆叠 props。

例如，虚拟表格只适合数据量较大、行结构适合虚拟化且滚动高度可控的场景。若只是缺少通用的展示或交互能力，先考虑增加向后兼容的 props；若业务需要与虚拟化假设冲突，则使用普通表格、AutoHeightTable 或新增专用组件，并说明选型理由。
